...
export default {
     ...
     * @param {Function} onPositiveClick
     * @param {Function} ele
     * @returns
     */
    popDel (text, onPositiveClick, ele){
        let ps = typeof(text)=='string'?{ text }: text
        return h(
            NPopconfirm,
            {
                onPositiveClick,
                "positive-button-props": {type: ps.type??"error"}
            },
            {
                default: ()=>h('div', {innerHTML: ps.text }),
                trigger: ele? ele:()=>UI.iconBtn(Trash, null, {type: ps.type??"error"})
            }
        )
    },
    /**
    ...
    menuItem (routeName, text, icon){
        ...
        return {
            ...
            key: to.name,
            icon: icon?buildIcon2(icon):undefined
        }
    },
    /**
