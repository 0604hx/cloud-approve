 ...
 * @Last Modified time: 2025-01-20 11:36:25
 *
 *
 * 注意：
 * ① 统一使用 json 提交数据
...
window.FETCH_JSON = (url, body, widthToken=false, method="post")=> {
    ...
    if(widthToken)  headers[import.meta.env.PUBLIC_HEADER_TOKEN] = localStorage.getItem(import.meta.env.PUBLIC_HEADER_TOKEN) || window.TOKEN || ""

    return fetch(url, {method, body:JSON.stringify(body), headers})
        .then(v=>v.json())
        .catch(e=>{
            if(!window.M)   return H.log.error(`请求 ${url} 出错：`, e)
 ...
 * @constructor
 */
window.POST=(url,data,onOk,onFail, headers={})=>{
    M.loadingBar.start()

    //提交数据到服务器
    axios.post(window.SERVER + url, data, {headers}).then(function (response) {
    ...
    POST(
        ...
        function (res) {
            ...
            if(res.success === true) onOk && onOk(res)
                ...
                if(res.message==="NOT_LOGIN"){
                    M.warn('请先登录再使用该功能')
                    return E.emit("jumpTo", {name:"login"})
                }
...
window.DOWNLOAD=(url, data, ps={})=>{
    ...
    //提交数据到服务器
    method(window.SERVER + url, ps.json?data:qs.stringify(data||{}), {responseType: 'blob', headers}).then(function (response) {
        ...
        if((!fileName || !contentDisposition) && response.data.type=="application/json"){
            ...
            fileName = fileName || ("文件下载-"+H.date.datetime(H.date.compact(), "YYYYMMDDHHmmss"))
            /**
             * 如果自定义了处理函数
             *
